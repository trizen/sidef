...
#>>>
    ...
    sub _fits_ulong {
        my ($n) = @_;
        ref($n) eq ''           and return ($n >= 0);
        ref($n) eq 'Math::GMPz' and Math::GMPz::Rmpz_fits_ulong_p($n);
    }

    sub _get_ulong {
        my ($n) = @_;
        ref($n) eq '' and return $n;
...
#>>>
    ...
    *fac = \&factorial;
    ...
    sub factorialmod {
        ...
        if (HAS_PRIME_UTIL and _fits_ulong($$n) and _fits_ulong($$m)) {
...
#>>>
    ...
    *Bellmod = \&bellmod;
    ...
    sub cubic_formula {
        ...
        my $R = $ONE;
        my $z = __div__(__sub__(__sqrt__(_any2mpc(-3)), $ONE), $TWO);
...
#>>>
                ...
                for (my $r ; $p <= $s ; $p = $r) {
                    ...
                    if (!($u > $v)) {
                        $p = $r if $squarefree;
                        $u = $p if ($k == 2 and $p > $u);
                        __SUB__->($t, $L, $p, $k - 1, ($k == 2) ? ($u, $v) : ());
                    }
                }
              }
              ->(1, 1, (($carmichael || $lucas_carmichael) ? 3 : 2), $k);
            ...
            sub {
              ...
              ->(
                 Math::GMPz::Rmpz_init_set_ui(1),
                 Math::GMPz::Rmpz_init_set_ui(1),
                 (($carmichael || $lucas_carmichael) ? 3 : 2), $k
                );
...
#>>>
    ...
    *is_fib = \&is_fibonacci;
    ...
    sub is_carmichael {    # OEIS: A002997
        ...
        if (_fits_ulong($n)) {
            $n = _get_ulong($n);
            return ((HAS_PRIME_UTIL ? Math::Prime::Util::is_carmichael($n) : Math::Prime::Util::GMP::is_carmichael($n))
                    ? Sidef::Types::Bool::Bool::TRUE
                    : Sidef::Types::Bool::Bool::FALSE);
        }
