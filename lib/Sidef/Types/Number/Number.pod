...
Both arguments must be nonnegative. If C<a> is greater than C<b>, then function will return a random integer in the range C<[b,a]>.
...
Takes a nonnegative integer C<n>, which is used to seed the internal CSPRNG used for random functions, such as C<urand> and the random prime functions.
