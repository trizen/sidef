...
It's considerably faster than using the C<next_prime(n)> method.
...
Returns C<exp(n)> normalized in the range C<[0,1)>. The base C<b> can be value > 1.
...
Efficiently compute the n-th Fibonacci number modulo C<m>.
...
This method is particularly effective for numbers that have factors closed to each other, or have a factor C<k> for which C<znorder(2,k)> is small.
