...
Binary search, returning the minimum integer value in the range C<a..b> that satisifes the given comparison function.
...
The method is implemented as C<base.digits2num(digits...)>, where digits are in reverse order.
...
Returns the "logarithm-root" of n, such that C<lgrt(n) ** lgrt(n) == n>.
...
Lucas C<U_n(P,Q) (mod m)> function.
...
Lucas C<V_n(P,Q) (mod m)> function.
...
It returns an array containing C<n> consecutive primes that are `>= start` (if omitted, then start = 2).
...
=head2 squarefree_usigma
    ...
    squarefree_usigma(n, k=1)

Sum of the unitary squarefree divisors of n, each divisor raised to power k.

Example:

    say squarefree_usigma(5040)     #=> 48
    say squarefree_usigma(5040, 2)  #=> 1300
...
=head2 sum_of_remainders

    sum_of_remainders(n, v)

Returns the following sum: C<Sum_{k=1..n} v % k>, computed in C<O(sqrt(v))> steps.

Example:

    say 20.of {|n| sum_of_remainders(n, n) }        #=> A004125
    say 20.of {|n| sum_of_remainders(n, n.prime) }  #=> A099726
...
Returns the number of squarefree divisors of n.

Equivalently, the number of unitary divisor of n.
...
The smallest positive integer k such that C<powmod(a, k, n) == 1>.
