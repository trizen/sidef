...
Return an array with the positive divisors of C<n> that are less than or equal to C<k>.
...
Returns prime factors of C<n> (with repetition), sorted in ascending order.
...
The block is expected to return an array with zero or more (not necessarily prime) factors of C<k>.
...
Computes the factorial of C<n>: C<1×2×3×...×n>.
    ...
    say 100!            #=> 93326215443944152681699...
...
Returns true if C<n> is a prime number.
    ...
    say is_prime(2**127 - 1)  #=> true (Mersenne prime)

For numbers up to 2^64, uses a deterministic Miller-Rabin test. For larger numbers, uses probabilistic B-PSW and additional tests, which have no known counter-examples.
...
Returns the golden ratio constant C<PHI>, or the Euler totient of C<n>.

    say Num.phi                #=> 1.618033988749...
    say 30.of {|n| phi(n) }    #=> OEIS: A000010
...
Counts the number of primes <= C<n>, or in the range C<[a,b]>.
...
For k>1, it returns the generalized Jordan's totient C<J_k(n)>.
...
The Euler–Riemann zeta function. (currently, defined only for real numbers)
