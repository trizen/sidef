...
This class inherits from L<Sidef::Object::Object>.
...
B<Unsafe example> (VULNERABLE):
    ...
    var filename = Sys.readln          # User input
...
1. B<Validate input> against a whitelist of allowed values

    var filename = Sys.readln.trim
...
2. B<Use File I/O instead> of shell commands when possible

    var filename = Sys.readln.trim
    var content = File(filename).read   # Safer - no shell involved

3. B<Use Sys.run with array arguments> for proper escaping

    var filename = Sys.readln.trim
    var content = Sys.run("cat", filename)

4. B<Escape shell metacharacters> (complex and error-prone)
