...
The C<Pipe> class provides an interface for working with Unix-style pipes in Sidef. Pipes allow communication between your Sidef program and an external command: data can flow from a command into your program (read mode) or from your program into a command (write mode).
...
This distinction matters: when the Pipe wraps a B<single> string, that string is handed to the shell for parsing (so shell metacharacters such as pipes, redirections and globs are interpreted). When the Pipe wraps B<multiple> strings, the first is executed directly as a program with the rest as its arguments, bypassing the shell entirely (safer, and immune to shell-injection issues).

The command isn't actually executed until C<open>, C<open_r>, or C<open_w> is called.
...
The command is not executed until C<open>, C<open_r>, or C<open_w> is called.
...
Opens the pipe using the given mode, executing the wrapped command as described in L</DESCRIPTION>. Valid modes are C<'-|'> for read (command output flows into your program) and C<'|-'> for write (your program's output flows into the command), optionally followed by a PerlIO layer such as C<':utf8'>.

If C<var_ref> is given, the resulting L<Sidef::Types::Glob::FileHandle> is stored into it, and C<open> returns the process ID of the spawned command (or C<nil> on failure). If C<var_ref> is omitted, C<open> returns the FileHandle directly instead (or C<nil> on failure).
...
Opens the pipe in read mode (equivalent to C<open('-|:utf8', var_ref)>), allowing your program to read the command's output. As with C<open>, passing C<var_ref> makes this return the process ID (with the FileHandle stored into C<var_ref>); omitting it returns the FileHandle directly.
...
Opens the pipe in write mode (equivalent to C<open('|-:utf8', var_ref)>), allowing your program to send data to the command's standard input. As with C<open>, passing C<var_ref> makes this return the process ID (with the FileHandle stored into C<var_ref>); omitting it returns the FileHandle directly.
...
Opens the pipe for reading, reads all of its output, and closes it — a one-call shortcut for C<open_r> + read-to-end + C<close>.

    say Pipe("ls -la").slurp
...
Opens the pipe for writing, sends C<data>, and closes it. Returns true on success, or C<nil> if the pipe couldn't be opened.

    Pipe("sort").write_all("banana\napple\n")
...
Returns a String representation of the Pipe object, showing each part of the wrapped command individually quoted, e.g. C<Pipe("grep", "pattern", "file.txt")>.
...
=head2 Getting the Filehandle Without a Var Reference
...
L<Sidef::Types::Glob::File>, L<Sidef::Types::Glob::FileHandle>, L<Sidef::Types::Glob::Dir>, L<Sidef::Sys::Sys>
