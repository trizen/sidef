...
Constructs a new DirHandle object that wraps a raw directory handle (as produced internally by C<opendir>) together with the directory it belongs to. This is a low-level constructor: DirHandle objects are normally obtained by calling C<open> on a L<Sidef::Types::Glob::Dir> object rather than being built directly.

Aliases: I<call>

=cut

=head1 DIRECTORY INFORMATION
...
=head1 READING ENTRIES

=head2 read

    self.read

Reads and returns the next entry from the directory as a L<Sidef::Types::Glob::Dir> or L<Sidef::Types::Glob::File> object, depending on the entry's type. Returns C<nil> when there are no more entries. Each call advances the directory position to the next entry.

The special entries C<.> and C<..> are automatically skipped, and symbolic links are silently ignored (they are not returned at all).
...
Returns an array containing all the entries in the directory, as L<Sidef::Types::Glob::Dir> and L<Sidef::Types::Glob::File> objects. Internally this rewinds the handle and then repeatedly calls C<read>, so, like C<read>, it excludes the C<.> and C<..> entries as well as symbolic links.
...
Returns the number of entries in the directory (excluding C<.> and C<..>).
...
Returns an array containing only the regular files within the directory (i.e., the result of C<entries> filtered down to entries where C<is_file> is true). Subdirectories, symbolic links, and the C<.> / C<..> entries are excluded.
...
Returns an array containing only the subdirectories within the directory (i.e., the result of C<entries> filtered down to entries where C<is_dir> is true). Regular files, symbolic links, and the C<.> / C<..> entries are excluded.
...
Unlike C<read>, C<entries>, C<files>, and C<dirs>, this method reads directly from the underlying directory handle: it does B<not> skip the C<.> and C<..> entries, and it does B<not> filter out symbolic links.
...
Returns an C<Array> of the results of calling C<block> on each entry (as produced by C<entries>).
...
Returns an C<Array> of entries (as produced by C<entries>) for which C<block> returns a truthy value.

    dh.grep {|f| f.is_file && f.match(/\.txt$/) }
...
Returns a callable iterator (a Block) that lazily yields the next raw entry name (as a string) each time it's invoked, returning C<nil> once the directory is exhausted. This is useful for processing large directories without loading all entries into memory at once.

Like C<each>, and unlike C<read>, this reads directly from the underlying directory handle, so it does B<not> skip the C<.> and C<..> entries and does B<not> filter out symbolic links.

    var dh = Dir('.').open
    var iterator = dh.iter

    # Get entries one at a time
    while (var entry = iterator.run) {
        say entry
    }
...
Recursively visits every entry in the directory tree rooted here, calling C<code> once per file or subdirectory encountered (depth-first). Returns the C<DirHandle>.
...
Returns a L<Sidef::Types::Glob::Stat> object containing filesystem metadata about the directory represented by this handle.
...
Returns a L<Sidef::Types::Glob::Stat> object containing filesystem metadata about the directory represented by this handle, following C<lstat> semantics.
...
C<read>, C<entries>, C<files>, and C<dirs> all skip the special C<.> and C<..> entries and silently ignore symbolic links. In contrast, C<each> and C<iter> read directly from the underlying directory handle and include C<.>, C<..>, and symbolic links, returning plain entry-name strings rather than Dir/File objects.
