...
Unlike direct subscript access (C<hash{key}>), C<fetch> allows you to distinguish between a missing key and a key whose value is nil.
...
Aliases: I<exists>, I<has_key>, I<haskey>, I<contain>, I<contains>, I<include>, I<includes>
...
Returns true if the hash has exactly the same set of keys as C<other> (another hash), false otherwise. Values are not compared.
...
Returns a new hash containing only the specified keys and their values from the original hash. Keys that do not exist in the original hash are included in the result with nil values.

    var h = Hash(a => 1, b => 2, c => 3, d => 4)
    var subset = h.slice(:a, :c, :z)
    say subset  #=> Hash(a => 1, c => 3, z => nil)

Note: Missing keys are preserved with nil values. See C<lsel> for the Array/to_list variant of this operation.
...
Performs a linear selection: returns a new hash containing the keys specified in C<keys_array>, including missing keys with nil values. The main difference from C<slice> is the argument shape: C<lsel> takes an Array (or any object responding to C<to_list>), while C<slice> takes variadic keys.

    var h = Hash(a => 1, b => 2)
    var selected = h.lsel([:a, :c])
    say selected  #=> Hash(a => 1, c => nil)

The argument must be an Array (or any object responding to C<to_list>).

Aliases: I<linsel>, I<linear_selection>

=cut

=head1 MODIFYING THE HASH
...
This is the in-place counterpart to C<grep>, which returns a new filtered hash instead of modifying the original.
...
Updates the values in the hash for keys that also exist in C<other_hash>. Only keys already present in the original hash are affected; no new keys are added. Modifies the hash in-place and returns it.
...
Note: Key C<:d> from C<h2> was not added because it did not already exist in C<h1>.
...
This is the non-destructive counterpart to C<delete_if>.
...
Note the difference from C<+> (merge): union keeps the left-hand value for common keys, while merge keeps the right-hand value.
...
Returns the hash as a tree structure represented as nested Pairs, using the given C<root> as the tree's root label. Nested hashes are recursively converted to sub-trees. Non-hash leaf values are represented as Pairs with an empty array C<[]>; only the key hierarchy is preserved, not the leaf values themselves.
