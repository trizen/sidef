...
Returns a new hash containing only the specified keys and their values from the original hash. Keys that do not exist in the original hash are omitted from the result.

    var h = Hash(a => 1, b => 2, c => 3, d => 4)
    var subset = h.slice(:a, :c, :z)
    say subset  #=> Hash(a => 1, c => 3)

Note: Key C<:z> was omitted because it does not exist in the original hash. See C<lsel> for a version that includes nil values for missing keys.
...
Performs a linear selection: returns a new hash containing the keys specified in C<keys_array>. Unlike C<slice>, keys that do not exist in the original hash are included in the result with nil values.
