...
Return only the elements which doesn't exists in both arrays.
...
Combine all the elements from the array togheter. The '+' method is used to concatenate all the elements together.
...
Returns true if the element from the I<Number> position is defined.
    ...
    [1,2,3,4,5].defined(0);     # true
    [1, 2, nil].defined(2);     # false
...
Alias for L<I<E<lt>E<lt>>|"E<lt>E<lt>">.

=cut

=head2 dropRight

Alias for L<I<E<gt>E<gt>>|"E<gt>E<gt>">.
...
Returns the index of the first element which passes the block test. Works just like the L<I<find()>|"find"> method, except that it returns the possition of the element, not the element itself.
...
Alias for L<I<firstIndex()>|"firstIndex">.
...
Alias for L<I<joinInsert()>|"joinInsert">.
