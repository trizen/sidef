...
The Array class exposes a large operator and method surface. The entries below are grouped by topic for easier browsing.

Within each category, methods are listed alphabetically, except for operators, which are grouped by related behavior.
...
This method analyzes the frequency of each distinct element in the array,
constructs a Huffman tree, and returns a hash mapping each element to its
corresponding Huffman code.

The resulting codes are canonical Huffman codes, meaning that symbols with
shorter code lengths receive lexicographically smaller binary codes, as
...
=head1 EXAMPLES
    ...
    var chunks = [1,2,3,4,5,6].segment(2)
