...
Sidef::Types::Array::Array - Dynamic ordered collection with rich functional, numeric, and utility methods.

=head1 DESCRIPTION

This class implements arrays in Sidef: ordered, mutable, zero-indexed collections of values. Arrays can hold elements of any type and provide operators plus methods for querying, searching, transforming, grouping, randomization, combinatorics, and matrix-oriented work.

=head1 SYNOPSIS

    var nums = Array(1, 2, 3, 4)
    var nums = [1, 2, 3, 4]
    var words = %w(alpha beta gamma)
    ...
    nums.map { _**2 }    #=> [1, 4, 9, 16, 25]
    nums.grep { _.is_even }   #=> [2, 4]
    nums.sum             #=> 15
...
Returns the valid index range of the array, from C<0> to C<arr.end>.

    [1, 5, 3, 9, 2].range    #=> 0..4

=cut

=head1 Element Access, Slicing & Partitioning
...
Returns the slice from index C<from> through index C<to>, inclusive. Negative indices count from the end of the array. If C<to> is omitted, it defaults to the last index.
...
Returns a slice of the array. The first element is at offset zero.

If C<offset> is negative, the slice starts that far back from the end of the array.

If C<length> is omitted, everything through the end of the array is returned.

If C<length> is negative, that many elements are left off the end of the array.
...
Returns a new array with consecutive duplicate elements removed, keeping only the first element of each run.

Example:

    say [1, 1, 2, 2, 3, 1, 1].squeeze    #=> [1, 2, 3, 1]

Aliases: I<squeeze_dup>

=cut

=head2 squeeze_by

    arr.squeeze_by { |prev, curr| ... }

Returns a new array with consecutive elements removed when the given block returns a true value for the previous and current element.

Example:

    say [1, 2, 2, 3, 4, 4, 4, 5].squeeze_by { |a, b| a == b }    #=> [1, 2, 3, 4, 5]

=cut

=head2 chunk_while

    arr.chunk_while { |prev, curr| ... }

Splits the array into an array of sub-arrays, starting a new chunk whenever the block returns a false value for two consecutive elements.

Example:

    say [1, 2, 4, 5, 7, 8].chunk_while { |a, b| b - a == 1 }    #=> [[1, 2], [4, 5], [7, 8]]
...
=head1 Searching, Membership & Binary Search
...
=head1 Iteration, Mapping & Reduction
...
Returns the running sums of the array. When a block is given, each element is mapped through the block first, and the cumulative sums are taken over the mapped values.

    [1,2,3,4].acc              #=> [1, 3, 6, 10]
    [1,2,3,4].acc { _**2 }     #=> [1, 5, 14, 30]
...
Returns the running sums of the values produced by the given block.
...
Returns the sum of the array, starting from C<initial>. This is equivalent to C<arr.reduce_op('+', initial)>.

    [1,2,3,4].collapse(0)    #=> 10
...
Maps over adjacent non-overlapping pairs of elements.
...
=head1 Filtering, Grouping & Uniqueness
...
Groups consecutive runs of equal elements.

    say <a a a b b c>.stack     #=> [["a", "a", "a"], ["b", "b"], ["c"]]

When a block is given, consecutive elements are grouped according to the values returned by the block:
...
Sorts the array by mapping each value through the given block and ordering by the mapped results.
...
=head1 Aggregation, Statistics & Number Theory
...
Returns the geometric mean of a list of numbers. The geometric mean is the n-th root of the product of n numbers, useful for averaging ratios and growth rates.
    ...
    say [42, 50, 99, 147].geometric_mean             #=> 74.352051512093712...
    say [42, 50, 99, 147].geometric_mean { _**2 }    #=> 5528.2275640570368...
...
Returns the harmonic mean of a list of numbers. The harmonic mean is the reciprocal of the arithmetic mean of reciprocals, useful for averaging rates.
    ...
    say [42, 50, 99, 147].harmonic_mean             #=> 65.883471411109602...
    say [42, 50, 99, 147].harmonic_mean { _**2 }    #=> 3586.7978615843786...
...
Returns the population variance of a list of numbers (average squared deviation from the mean; divides by C<n>, not C<n-1>).
    ...
    say [1,2,3,4].variance            #=> 1.25
    say [1,2,3,4].variance { _**2 }   #=> 32.25
...
Returns the population standard deviation — the square root of L</variance>.
    ...
    say [1,2,3,4,5].stddev            #=> sqrt(2)
    say [1,2,3,4,5].stddev { _**2 }   #=> sqrt(74.8)
...
=head1 Combinatorics & Randomization
...
Returns one random element, or C<n> distinct random elements.
...
Uses C<next_permutation> to generate all unique permutations of the array.

    say [1,1,2].unique_permutations         #=> [[1, 1, 2], [1, 2, 1], [2, 1, 1]]

This is equivalent to C<arr.permutations.uniq>, but more efficient because duplicate permutations are never generated.

The method also accepts an optional callback block:
...
=head1 Sequence Analysis, Similarity & Compression
...
Builds a Huffman code table, given an array of Number objects.

This method analyzes the frequency of each distinct element in the array, constructs a Huffman tree, and returns a hash mapping each element to its corresponding Huffman code.

The resulting codes are canonical Huffman codes, meaning that symbols with shorter code lengths receive lexicographically smaller binary codes, as described in RFC 1951.

Each Huffman code is represented as a String object of C<0> and C<1> characters.

    var arr = [1, 1, 1, 2, 2, 3]
    var codes = arr.huffman

    say codes{1}   # e.g. "0"
    say codes{2}   # e.g. "10"
    say codes{3}   # e.g. "11"

(The exact codes depend on symbol frequencies.)
...
=head1 Operator Helpers, Zipping & Element-wise Operations

=head2 combine

    arr.combine { |a,b| ... }

Recursively combines corresponding elements from nested arrays using the given block.
...
Zips an array of arrays together, truncating to the shortest input array. Without a block, it returns an array whose elements contain the corresponding items from each input array.

    say [[1,2,3], [4,5,6]].zip                #=> [[1, 4], [2, 5], [3, 6]]
    say [[1,2,3], [4,5,6], [7,8,9]].zip       #=> [[1, 4, 7], [2, 5, 8], [3, 6, 9]]
...
Zips an array of arrays together and applies the given block to each group of corresponding elements.

    say [[1,2,3], [4,5,6]].zip_by({|a,b| a+b })      #=> [5, 7, 9]

=cut

=head2 zip_op

    arr.zip_op(operator, other_arr)

Zips C<arr> with C<other_arr> and applies the given operator to each pair of corresponding elements.
...
=head1 Conversion, Joining & Encoding
...
Joins array elements into a string with an optional delimiter. When a block is given, each element is mapped through the block before joining.
...
=head1 EXAMPLES
    ...
    var grouped = %w(a aa bbb ccc dd).group_by { _.len }
    say grouped

    # Build 2-element chunks
    var chunks = [1, 2, 3, 4, 5, 6].slices(2)
    say chunks         #=> [[1, 2], [3, 4], [5, 6]]

    # Keep unique values in order
    var uniq = [3, 1, 3, 2, 1, 4].uniq
    say uniq           #=> [3, 1, 2, 4]

=head1 SEE ALSO

L<Sidef::Types::Array::Matrix>, L<Sidef::Types::Array::Vector>, L<Sidef::Types::Hash::Hash>, L<Sidef::Types::Set::Set>
