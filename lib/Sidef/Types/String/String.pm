...
sub bytes {
    ...
    my $string = $$self;
    require bytes;
    Sidef::Types::Array::Array->new([map { Sidef::Types::Number::Number::_set_int(CORE::ord(bytes::substr($string, $_, 1))) } 0 .. bytes::length($string) - 1]);
}
...
sub from_json {
    ...
    Sidef::Types::Perl::Perl->to_sidef(scalar JSON::PP::decode_json($$self));
}
