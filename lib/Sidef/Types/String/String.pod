...
Sidef::Types::String::String - Unicode string type with comprehensive text processing and manipulation methods.
...
Example:
    ...
    if ("testing" =~ /(\w+)ing/) {|m| say m[0] }  #=> "test"
...
Returns a new string with the first character converted to uppercase. The rest of the string is left unchanged. This is the counterpart of C<lcfirst>. To title-case every word in a string, use C<wc> instead.
...
Returns a new string with the case of every letter swapped: uppercase letters become lowercase and lowercase letters become uppercase. Non-letter characters are left unchanged. Unicode letters are handled correctly.

Example:

    say "Hello World".swapcase    #=> "hELLO wORLD"
    say "ABC def".swapcase        #=> "abc DEF"
...
Returns true if the string is non-empty and consists entirely of lowercase letters, false otherwise. Any non-letter character (including digits or whitespace) causes it to return false.
...
Returns true if the string is non-empty and consists entirely of uppercase letters, false otherwise. Any non-letter character (including digits or whitespace) causes it to return false.
...
Returns true if all characters in the string are ASCII characters (code points 0-127) and the string is not empty, false otherwise.
...
Returns true if the string is non-empty and consists entirely of blank characters (space or tab), false otherwise. Note that other whitespace characters (such as newlines) are not considered blank.
...
If the string starts with the given prefix, returns a new string with that prefix removed from the beginning. If the string does not start with the prefix, returns the string unchanged.

Example:

    say "hello world".delete_prefix("hello ")    #=> "world"
    say "hello world".delete_prefix("xyz")       #=> "hello world"

Aliases: I<remove_prefix>
...
If the string ends with the given suffix, returns a new string with that suffix removed from the end. If the string does not end with the suffix, returns the string unchanged.

Example:

    say "hello world".delete_suffix(" world")    #=> "hello"
    say "hello world".delete_suffix("xyz")       #=> "hello world"

Aliases: I<remove_suffix>

=cut

=head2 common_prefix

    str.common_prefix(other)

Returns the longest common prefix shared between C<self> and C<other> as a new string. Returns an empty string if the two strings share no common prefix.

Example:

    say "interspecies".common_prefix("interstellar")    #=> "inters"
    say "foo".common_prefix("bar")                       #=> ""

=cut

=head2 common_suffix

    str.common_suffix(other)

Returns the longest common suffix shared between C<self> and C<other> as a new string. Returns an empty string if the two strings share no common suffix.

Example:

    say "testing".common_suffix("boxing")    #=> "ing"
    say "foo".common_suffix("bar")           #=> ""
...
Word-wraps the string to at most C<width> characters per line, breaking only on whitespace between words. Returns an C<Array> of C<String> lines. Words longer than C<width> are placed alone on their own line without being split.

Example:

    say "The quick brown fox jumps over the lazy dog".wrap(20)
...
Checks whether C<str1> and C<str2> share a matching character at the same position (index). Returns C<true> as soon as one such position is found, C<false> otherwise. The two strings are XOR-ed together with the shorter one padded with NUL bytes, so positions beyond the shorter string are also considered (and can match if the longer string contains a NUL character there).

Example:

    say "hello".overlaps("world")    #=> true (both have 'l' at index 3)
    say "abc".overlaps("xyz")        #=> false

=cut

=head2 jaro

    str1.jaro(str2, winkler=false)

Calculates the Jaro similarity between two strings. The Jaro distance is a measure of similarity between two strings, based on the number of matching characters and transpositions. The result is a value between 0 and 1, where 1 indicates identical strings.

When the C<winkler> argument is true, it returns the Jaro-Winkler similarity score instead, which gives more weight to strings that match from the beginning.

Example:

    say "accommodate".jaro("accomodate")         #=> 0.96969696969697
    say "accommodate".jaro("accomodate", true)   #=> 0.981818181818182
...
Example:
    ...
    say "kitten".lev("sitting")    #=> 3
    say "hello".lev("hallo")       #=> 1

Aliases: I<leven>, I<levenshtein>

=cut

=head1 TRIMMING & PADDING
...
=head2 pad_left
    ...
    str.pad_left(width, padstr=" ")

Left-justifies the string within a field of the given width by padding the right side with the specified padding string (default is a single space). Only the first character of C<padstr> is used; multi-character padding strings are not repeated. If the string is already at or beyond the given width, it is returned unchanged.

Example:

    say "hi".pad_left(5)          #=> "hi   "
    say "hi".pad_left(5, "*")     #=> "hi***"
    say "hello".pad_left(3)       #=> "hello"
...
=head2 pad_right
    ...
    str.pad_right(width, padstr=" ")

Right-justifies the string within a field of the given width by padding the left side with the specified padding string (default is a single space). If the string is already at or beyond the given width, it is returned unchanged.

Example:

    say "hi".pad_right(5)         #=> "   hi"
    say "hi".pad_right(5, "0")    #=> "000hi"
    say "hello".pad_right(3)      #=> "hello"
...
Returns an empty string, regardless of the content of the invocant.
...
=head1 ACCESSING CHARACTERS & SUBSTRINGS

=head2 char

    str.char(pos)

Returns the character at the specified position in the string. Position indexing starts at 0. Returns an empty string if the position is out of bounds.
...
Returns the Unicode code point value of the first character in the string. Returns nil if the string is empty.
...
Returns the byte value at the specified position in the string. Position indexing starts at 0. Returns nil if the position is out of bounds.
...
Returns an Array with the individual digits found in the string, as numbers.
...
Returns the last character of the string. If the string is empty, returns an empty string.
...
=head1 SPLITTING, JOINING & SORTING
...
Returns a new C<RangeString> object as C<RangeStr(from, to, -step)>.
...
=head1 SUBSTITUTION & MODIFICATION
...
=head1 PATTERN MATCHING & PARSING
...
Applies the Burrows–Wheeler transform (BWT) to the invocant string and returns a pair consisting of the transformed string and the primary index.

The primary index is required for decoding with C<bwt_decode>.
...
Reverses the Burrows–Wheeler transform and reconstructs the original string.

This method takes a BWT-transformed string and its associated primary index (as returned by C<bwt_encode>) and reconstructs the original string using the standard LF-mapping technique.

    var original = bwt.bwt_decode(idx)

=cut

=head1 HASHING & CRYPTOGRAPHY
...
=head1 FORMATTING & OUTPUT

=head2 format

    str.format

Interprets the invocant as a Perl "format" declaration (the same syntax used with Perl's built-in C<write> function) and returns the generated output as a new string. This is unrelated to sprintf-style formatting; for that, see C<%>.

Example:

    say "Hello, World!".format    #=> "Hello, World!"
...
Extracts values from C<str> according to the template format specified by the invocant. This is the reverse of the C<pack> operation.

Example:

    say "C*".unpack("Hi")    #=> [72, 105]

=cut

=head1 SYSTEM & EXECUTION
