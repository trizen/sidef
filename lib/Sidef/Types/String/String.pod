...
Checks whether C<str1> and C<str2> share a matching character at the same position (index). Returns C<true> as soon as one such position is found, C<false> otherwise. Only the positions up to the length of the shorter string are compared.
