...
Returns a substring of the input string that starts at the specified C<from> index and ends at the specified C<to> index.

Parameters:

    from : An integer that represents the index at which to start the substring.
    to   : An integer that represents the index at which to end the substring.
