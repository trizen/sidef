...
Given an integer and a list of numbers, it tries to find non-trivial factors of n, checking each gcd(n, array[0]), gcd(n, array[1]), etc.

    var n = 43*43*97*503
    var a = [19*43*97, 1, 13*41*43*101]
    say Math.gcd_factors(n, a)          #=> [43, 97, 503]

When no factors are found, an empty list is returned. The returned list contains no duplicates.
