...
=item * and a reasonable number of built-in types.
...
=head1 SYNOPSIS
    ...
    # Deparse the AST back into Sidef code
    my $deparsed = $sidef->compile_ast($ast, 'Sidef');
...
Parses a given string as Sidef code and returns the AST.
...
Returns a new optimized AST. The original AST is not altered.
...
Valid values for C<$backend> are: C<'Sidef'>, C<'Perl'>.
...
Parses, compiles and executes a given Sidef code and
returns the result of the last statement. (recommended)

=head2 execute_perl

    my $result = $s->execute_perl($code);

Executes compiled Perl code and returns the result. (discouraged)
...
=item * Gitbook: L<http://trizen.gitbooks.io/sidef-lang/content/>

=item * Tutorial: L<https://github.com/trizen/sidef/wiki>

=item * RosettaCode: L<http://rosettacode.org/wiki/Sidef>
